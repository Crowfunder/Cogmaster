...
[Group(CommandIds.Translations, "Resolves and reverse-searches names from i18n properties.")]
...
{
    [SlashCommand("value", "Get the property names that match the given value.")]
    public async Task ValueCommand([Summary(description: "Reverse search for property value, i.e \"Brandish\""), Autocomplete(), MinLength(3), MaxLength(69)] string value) =>
        await HandleCommandAsync($"{DotNetEnv.Env.GetString("api")}/properties/value?q={value}", value, "No properties matching the value.");

    [SlashCommand("key", "Translate the given key.")]
    public async Task KeyCommand([Summary(description: "Resolve property name, i.e \"m.brandish\"."), MinLength(3), MaxLength(69)] string key) =>
        await HandleCommandAsync($"{DotNetEnv.Env.GetString("api")}/properties/key?q={key}", key, "Failed to find property.");
